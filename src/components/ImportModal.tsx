...
export function ImportModal({ open, onClose }: ImportModalProps) {
  const { state, setSelectedComponents, setComponentConfidence, addFiles } = useApp();
  ...
  const handleAnalyze = () => {
    if (files.length > 0 && state.selectedComponents.length > 0) {
      addFiles(files);
      onClose();
      setFiles([]);
    }
  ...
  const canAnalyze = files.length > 0 && selectedCount > 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#252526] border border-[#3c3c3c] rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-[#3c3c3c] bg-[#1e1e1e]">
          <h2 className="text-xl font-bold text-white">Import Drawings</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-[#858585]">{selectedCount} of {readyComponents.length} selected</span>
            </div>
          ...
          <section>
            <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4">Step 2: Upload PDF Files</h3>
            <div
              onDragEnter={handleDrag}
              onDragLeave={handleDrag}
              onDragOver={handleDrag}
              onDrop={handleDrop}
              className={cn('border-2 border-dashed rounded-lg p-12 text-center transition-all', dragActive ? 'border-[#007acc] bg-[#007acc]/10' : 'border-[#3c3c3c] bg-[#1e1e1e] hover:border-[#858585]')}
            >
              <Upload className={cn('w-12 h-12 mx-auto mb-4', dragActive ? 'text-[#007acc]' : 'text-[#858585]')} />
              ...
              <label className="inline-block bg-[#007acc] hover:bg-[#0062a3] text-white px-6 py-2 rounded cursor-pointer transition-colors font-semibold">
                Browse Files
                  ...
                  {files.map((file, index) => (
                    <div key={index} className="flex items-center justify-between bg-[#1e1e1e] border border-[#3c3c3c] rounded p-2 hover:bg-[#25272e] transition-colors">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <FileText className="w-4 h-4 text-[#007acc] shrink-0" />
                        <span className="text-sm text-white truncate">{file.name}</span>
                        <span className="text-xs text-[#858585] shrink-0">{(file.size / 1024 / 1024).toFixed(2)} MB</span>
                      </div>
                      <button onClick={() => removeFile(index)} className="p-1 hover:bg-[#3c3c3c] rounded transition-colors text-[#858585] hover:text-white shrink-0">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
        ...
        <div className="flex items-center justify-between p-5 border-t border-[#3c3c3c] bg-[#1e1e1e]">
          <div className="text-sm text-[#858585]">
            {canAnalyze ? (
              <span>Ready to analyze <span className="text-white font-semibold">{files.length}</span> file(s) with <span className="text-white font-semibold">{selectedCount}</span> component(s)</span>
            ) : (
              <span>Select components and upload files to continue</span>
            )}
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={handleAnalyze} disabled={!canAnalyze} className={cn('px-6 py-2 text-sm font-semibold rounded transition-colors', canAnalyze ? 'bg-[#007acc] hover:bg-[#0062a3] text-white' : 'bg-[#3c3c3c] text-[#858585] cursor-not-allowed')}>
              Analyze {selectedCount > 0 && `${selectedCount} Component${selectedCount > 1 ? 's' : ''}`}
            </button>
          </div>
        </div>
      </div>
